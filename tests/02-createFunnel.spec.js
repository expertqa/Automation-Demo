...
test('TC-02 — Funnel, Idea & Kanban', async ({ page }) => {
  ...
  await test.step('TC-02.1 Create a new idea funnel', async () => {
    ...
    expect(funnelName, 'createFunnel() should return the funnel name it used').toBeTruthy();
    await expect(page, 'Should land on the funnel\'s Kanban view').toHaveURL(/\/studio\/funnels\/\d+\?view=kanban/);
  });

  await test.step('TC-02.2 Create an idea inside the funnel', async () => {
    ({ ideaName: ideaTitle } = await ideaPage.createIdea());

    expect(ideaTitle, 'createIdea() should return the idea name it used').toBeTruthy();
  });
