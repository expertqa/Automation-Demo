...
test("TC-01 — Login", async ({ page }) => {
  test.setTimeout(30000 + RATE_LIMIT_TIME_BUDGET_MS);
  ...
  await test.step("TC-01.1 Login with empty email and password", async () => {
    ...
    await expect(
      page,
      "Empty credentials should be rejected — user stays on /login",
    ).toHaveURL(/\/login/);
  });
  ...
  await test.step("TC-01.2 Login with invalid credentials", async () => {
    ...
    await expect(
      page,
      "Invalid credentials should be rejected — user stays on /login",
    ).toHaveURL(/\/login/);
    await expect(
      page
        .getByText(/We could not sign you in|Login failed/)
        .locator("visible=true")
        .first(),
      "An invalid-login error message should be shown",
    ).toBeVisible();
  });
  ...
  await test.step("TC-01.3 Login with valid credentials", async () => {
    ...
    await expect(
      page,
      "Valid credentials should log the user in and leave /login",
    ).not.toHaveURL(/\/login/);
  });
});
