...
export class CampaignPage {
  constructor(page) {
    ...
    // Navigation
    this.campaignsLink = page.getByRole('link', { name: 'Campaigns', exact: true }).first();
    this.addCampaignLink = page.getByRole('link', { name: 'Add campaign' });

    // "New campaign" required-setup dialog.
    this.newCampaignDialog = page.getByRole('dialog').filter({ hasText: 'Create campaign' });
    ...
    this.pickDateButton = this.newCampaignDialog.getByRole('button', { name: 'Pick a date' });
    this.todayButton = page.getByRole('button', { name: 'Today', exact: true });
    this.createCampaignSubmitButton = this.newCampaignDialog.getByRole('button', { name: 'Create campaign' });

    // General tab (Manage campaign page)
    this.campaignDescriptionBox = page.getByText('Campaign Description', { exact: true }).locator('xpath=following::textarea[1]');
    this.briefingBox = page.locator('div').filter({ hasText: /^Briefing for participants/ }).locator('[contenteditable="true"]').first();
    this.savedIndicator = page.getByText(/Saved at/);

    // Campaigns list
    this.loadingSpinner = page.locator('.animate-spin, [class*="spinner"]').first();

    // Tabs
    this.tab = (name) => page.getByRole('tab', { name, exact: true });

    // Planning tab
    this.softDeadlineCard = page.getByRole('button', { name: /^Soft deadline/ });
    ...
    this.addFieldDialog = page.getByRole('dialog').filter({ hasText: 'Manage form' });
    ...
    // Access tab
    this.accessCombobox = page.getByRole('combobox').filter({ hasText: 'Open' });
    this.addUsersButton = page.getByRole('button', { name: 'Add users' });
    this.addUsersDialog = page.getByRole('dialog').filter({ hasText: 'Add users' });
    // Row 0 is the header row; the first real user is row 1.
    this.firstUserRow = this.addUsersDialog.getByRole('row').nth(1);
    this.addUsersSubmitButton = this.addUsersDialog.getByRole('button', { name: 'Add users' });

  
    this.addCommunicationButton = page.getByRole('button', { name: 'Add communication' });
    this.newCommunicationDialog = page.getByRole('dialog').filter({ hasText: 'New communication' });
    this.selectUsersCombobox = this.newCommunicationDialog.getByText('Select users', { exact: true });

    // Auto responses tab
    this.submissionConfirmationCombobox = page.getByRole('combobox').filter({ hasText: /None|Browser only|Email only|Browser \+ Email/ });

    // Context tab
    this.overviewBox = page.getByText('Overview', { exact: true }).locator('xpath=following::textarea[1]');
    this.problemBox = page.getByText('Problem', { exact: true }).locator('xpath=following::textarea[1]');
  ...
  async createCampaign() {
    ...
    await this.newCampaignTitleInput.waitFor({ state: 'visible', timeout: 30000 });
    ...
    await this.newCampaignDialog.waitFor({ state: 'detached', timeout: 30000 });
  ...
  async fillCampaignDetails() {
    ...
    await this.campaignDescriptionBox.waitFor({ state: 'visible', timeout: 30000 });
    await this.campaignDescriptionBox.click();
    await this.campaignDescriptionBox.fill(description);

    await fillRichText(this.page, this.briefingBox, briefing, 'campaign-briefing');

    // Autosave confirmation, not an explicit Save button on this page.
    await this.savedIndicator.waitFor({ state: 'visible', timeout: 15000 });
  ...
  async configurePlanning() {
    ...
    await this.softDeadlineCard.waitFor({ state: 'visible', timeout: 15000 });
  ...
  async addCampaignTask() {
    ...
    await this.quickAddTaskInput.waitFor({ state: 'visible', timeout: 15000 });
    await this.quickAddTaskInput.fill(taskTitle);
    await this.quickAddTaskInput.press('Enter');
  
    await this.page.locator(`tr:has(input[value="${taskTitle}"])`).waitFor({ state: 'visible', timeout: 15000 });
  ...
  async addFormField() {
    ...
    await this.addFieldButton.waitFor({ state: 'visible', timeout: 15000 });
    await this.addFieldButton.click();

    await this.addDepartmentFieldButton.waitFor({ state: 'visible', timeout: 15000 });
  ...
  async configureAccess() {
    ...
    await this.addUsersButton.waitFor({ state: 'visible', timeout: 15000 });
    await this.addUsersButton.click();

    await this.firstUserRow.waitFor({ state: 'visible', timeout: 15000 });
    await this.firstUserRow.click();
    await this.addUsersSubmitButton.click();
    await this.addUsersDialog.waitFor({ state: 'detached', timeout: 15000 });
  ...
  async openCampaignCommunication() {
    ...
    await this.addCommunicationButton.waitFor({ state: 'visible', timeout: 15000 });
    await this.addCommunicationButton.click();

    await this.selectUsersCombobox.waitFor({ state: 'visible', timeout: 15000 });
    await this.page.keyboard.press('Escape');
    await this.newCommunicationDialog.waitFor({ state: 'detached', timeout: 15000 });
  ...
  async configureAutoResponses() {
    ...
    await this.submissionConfirmationCombobox.waitFor({ state: 'visible', timeout: 15000 });
    await this.submissionConfirmationCombobox.click();
    await this.page.getByRole('option', { name: 'Browser only', exact: true }).click();
    await this.savedIndicator.waitFor({ state: 'visible', timeout: 15000 });
  ...
  async fillCampaignContext() {
    ...
    await this.overviewBox.waitFor({ state: 'visible', timeout: 15000 });
  ...
  async uploadCampaignFile(filePath = 'test-data/logo.jpg') {
    ...
    await this.page.getByText('No files or links yet.').waitFor({ state: 'hidden', timeout: 20000 }).catch(() => {});
  ...
  async verifyCampaignInList(campaignTitle) {
    ...
    await campaignEntry.waitFor({ state: 'visible', timeout: 20000 });
