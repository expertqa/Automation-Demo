...
export class ProjectPage {
  constructor(page) {
    ...
    // already visible.
    this.searchProjectsTextbox = page.getByRole("textbox", {
      name: "Search projects",
    });
    this.projectDropdown = page.getByRole("combobox").first();
    this.selectProjectFunnel = page.getByRole("option", {
    ...
    this.projectDescriptionBox = page
    ...
    this.createTagButton = (tagName) =>
      page.getByRole("button", { name: `Create ${tagName}` });
    ...
    this.suggestedTagsHeading = page.getByText("Suggested tags", {
      exact: true,
    });
    this.suggestedTagButtons = this.suggestedTagsHeading
      .locator("xpath=following::button")
      .filter({ hasNotText: "Recalculate" });
    this.removeTagButton = (tagName) =>
      page.getByRole("button", { name: `Remove ${tagName}` });
    ...
    // in constructor
    this.leadDepartmentField = page
      .locator("div")
      .filter({ hasText: /^Lead department/ });
    this.leadDepartmentButton = this.leadDepartmentField.getByRole("button", {
      name: "Select department",
    });

    this.participatingDepartmentField = page
      .locator("div")
      .filter({ hasText: /^Participating departments/ });
    this.participatingDepartmentButton =
      this.participatingDepartmentField.getByRole("button", {
        name: "Select department",
      });
    ...
    this.addProblemButton = page
      .locator("button")
      .filter({ hasText: /^Add problem$/ });
    ...
    this.problemStatement = page.getByRole("textbox", {
    ...
    this.experimentTitle = page.getByRole("textbox", {
    ...
    this.notesField = page.getByRole("textbox", {
    ...
    this.addDecisionButton = page
      .locator("button")
      .filter({ hasText: /^Add decision$/ });
    ...
    this.addLessonRecommend = page.getByRole("textbox", {
    ...
    this.internalTeamButton = page.getByRole("button", {
      name: "Internal Team",
    });
  ...
  async createProject() {
    ...
    await fillRichText(
      this.page,
      this.projectDescriptionBox,
      generateRandomName("ProjectDes"),
      "project-description",
    );
  ...
  async toggleSuggestedTag() {
    await this.suggestedTagsHeading.waitFor({
      state: "visible",
    });
  ...
  async fillDetailsSection() {
    ...
    // fillDetailsSection
    await this.leadDepartmentButton.click();
