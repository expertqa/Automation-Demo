...
export class KanbanPage {
    constructor(page) {
        this.page = page;
        this.ideaStatusDropdown = page.getByRole('combobox').filter({ hasText: /Submitted|Active|Review|Planned/ }).first();
        ...
        async dragIdeaToLane(itemTitle, targetLaneName) {
            ...
            const { x: sourceX, y: sourceY } = this.getCenterCoords(await sourceCard.boundingBox());
            const { x: targetX, y: targetY } = this.getCenterCoords(await targetLane.boundingBox());
            ...
            await targetLane.getByText(itemTitle, { exact: false }).waitFor({ state: 'visible', timeout: 20000 });
        }

        async moveIdeaToStage1(itemTitle) {
            const lanes = ['Review idea', 'Create proposal', 'Approved proposal', 'Denied proposal'];
